...
class DataLoader:
    def __init__(self, task_dir, proportion):
        ...
        with open(os.path.join(task_dir, 'relations.txt')) as f:
            # 创建一个空字典，并赋值给self.relation2id属性，表示关系到编号的映射关系
        ...
        with open(os.path.join(self.trans_dir, 'circ_list.txt')) as f:
            self.circ_list = dict()
            for line in f:
                entity, eid = line.strip().split('\t')
                self.circ_list[entity] = int(eid)

        with open(os.path.join(self.trans_dir, 'disease_list.txt')) as f:
            self.disease_list = dict()
            for line in f:
                entity, eid = line.strip().split('\t')
                self.disease_list[entity] = int(eid)

        # 遍历从0到self.relation2id字典的长度（即关系数量）的整数序列，每个整数作为一个关系编号，并赋值给i变量
        ...
        self.tra_train = self.read_triples(self.trans_dir, 'train.txt',proportion)
        self.tra_valid = self.read_triples(self.trans_dir, 'valid.txt',1)
        self.tra_test = self.read_triples(self.trans_dir, 'test.txt',1)

        # 调用自定义的read_triples方法，传入归纳知识图谱数据集目录和'train.txt'文件名以及'inductive'模式作为参数，
        # 返回归纳训练集三元组列表，并赋值给self.ind_train属性
        self.ind_train = self.read_triples(self.ind_dir, 'train.txt',proportion)
        self.ind_valid = self.read_triples(self.ind_dir, 'valid.txt',1)
        self.ind_test = self.read_triples(self.ind_dir, 'test.txt',1)
    ...
    def read_triples(self, directory, filename, proportion):
        triples = []
        ...
        # If proportion is 1, return all triples without change
        if proportion == 1.0:
            return triples

        # If proportion < 1, randomly sample the triples
        if 0 < proportion < 1.0:
            # Calculate the number of samples to take
            num_samples = int(len(triples) * proportion)
            # Use random.sample to get a list of unique elements chosen randomly from the sequence
            triples_new = random.sample(triples, num_samples)
            return triples_new
