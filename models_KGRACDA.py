...
# 遍历从0到self.n_layer（图神经网络层数）的整数序列，每个整数作为一个层编号，并赋值给i变量
        for i in range(self.n_layer):
            ...
            # 调用self.gnn_layers列表中第i个元素（即GNNLayer对象）的forward方法，传入q_sub、q_rel、hidden、edges、nodes.size(0)（节点数量）和old_nodes_new_idx参数作为参数，返回新的隐藏状态矩阵，并赋值给hidden变量
            hidden = self.gnn_layers[i](q_sub.cuda(), q_rel.cuda(), hidden.cuda(), edges.cuda(), nodes.size(0), old_nodes_new_idx).cuda()
